...
public class CookieOrderTest {
	...
	public void testClimbInfo() {
		...
		assertEquals(500, cookie.getNumberOfBoxes());

		// test setters and getters
		cookie.setNumberOfBoxes(33);
		assertEquals(33, cookie.getNumberOfBoxes());
	...
	/* 3. Add a method to MasterOrder that allows you to add a CookieOrder to it's List. */
	 ...
	 assertEquals(3, masterOrder.getOrders().get(0).getNumberOfBoxes());
	 ...
	 * PART B
	 * 
	 * The removeVariety method updates the master order by removing all of the cookie orders in which the variety of cookie matches the parameter cookieVar. The master order may
	 * contain zero or more cookie orders with the same variety as cookieVar. The method returns the total number of boxes removed from the master order.
	 */
	 @Test
